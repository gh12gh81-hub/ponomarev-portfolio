...
- загрузка обложки, hero, изображений и видео галереи в Cloudinary;
- смешанная галерея из фото и видео с необязательным постером;
- изменение порядка медиафайлов перетаскиванием или стрелками;
- сохранение `public/data/projects.json` через GitHub Contents API;
- публикация изменений на сайте напрямую из GitHub без обязательного нового деплоя.
...
`CLOUDINARY_API_SECRET` используется только серверной функцией и никогда не отправляется в браузер. `VITE_CLOUDINARY_CLOUD_NAME` оставьте для отображения изображений и видео на публичном сайте.

Медиафайлы админки сохраняются в папках `portfolio/projects/<slug>`. Обложка и hero принимают только изображения. В галерею можно загружать изображения до 25 МБ и видео MP4, WebM или MOV до 100 МБ.
...
При сохранении админка обновляет `public/data/projects.json` отдельным GitHub-коммитом. Публичный API читает актуальный файл из GitHub на сервере и кэширует ответ на 30 секунд, поэтому новый проект появляется без обязательного Vercel deployment. При временной недоступности GitHub сайт автоматически использует JSON, встроенный в последний deployment.
